// Binary gh is the stdin/v1 filter for the `gh` command.
...
func main() {
	...
	if err := json.NewDecoder(os.Stdin).Decode(&req); err != nil {
		fmt.Fprintf(os.Stderr, "gh: decode request: %v\n", err)
		os.Exit(1)
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "gh: unknown operation %q\n", req.Operation)
		os.Exit(1)
	}

	if err := json.NewEncoder(os.Stdout).Encode(resp); err != nil {
		fmt.Fprintf(os.Stderr, "gh: encode response: %v\n", err)
		os.Exit(1)
	}
}
